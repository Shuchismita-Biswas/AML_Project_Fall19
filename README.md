...
![RP](RP.PNG)
> Fig. 1: Procedure for constructing unthresholded RP images from time-series data, reproduced from \cite{Hatami2017ClassificationOT}. On the left panel, we show a simple univariate time series $f(t)$ with 12 samples. The middle panel shows its two dimensional phase space trajectory with delay embedding 1. The dots are system states such that $s_i:(f(i),f(i+1))$. The right panel shows the unthresholded RP for $f(t)$. It is a $11\times11$ matrix, whose $(i,j)$-th entry is the euclidean distance between $s_i$ and $s_j$ in the phase space.
